...
We will be using `buildah` clusterTasks, which gets installed along with Operator. The operator installs few ClusterTask which you can see.
...
A pipeline defines tasks that should be executed and how they interact with each other via their inputs and outputs.
...
spec:
  ...
  - name: update-deployment
    ...
    - apply-manifests
```
Once you deploy the pipelines, you should be able to visualize pipeline flow in the OpenShift Web Console by switching over to the **Developer** perspective of the OpenShift Web Console. Select the pipeline tab, select project as `pipelines-tutorial` and click on pipeline `build-and-deploy`
...
repository or the image registry it will be pushed to in the pipeline. That's because pipelines in Tekton
are designed to be generic and reusable across environments and stages through
...
Above we have started `build-and-deploy` pipeline, with relevant pipeline resources to deploy the backend/frontend applications using a single pipeline
...
You can get the route of the application by executing the following command and accessing the application
...
* Run the below command to expose EventListener service as a route
